...
export default function CollaboratorModal({ note, onClose, onUpdate }) {
  ...
  const handleInvite = async (e) => {
    ...
    try {
      const res = await notesAPI.invite(note._id, { email, permission });
  ...
  const handleRemove = async (userId) => {
    try {
      ...
      onUpdate({ ...note, collaborators: note.collaborators.filter(c => c.user._id !== userId) });
      ...
      <div className="modal-card" onClick={e => e.stopPropagation()}>
        ...
        <form className="modal-form" onSubmit={handleInvite}>
          <div className="modal-field-row">
            <input type="email" placeholder="Invite by email…" value={email} onChange={e => { setEmail(e.target.value); setError(''); setSuccess(''); }} required />
          ...
          <p className="invite-note">They'll receive a notification and email to accept or decline.</p>
        </form>
        ...
        <div className="modal-section">
          ...
          <div className="collab-item">
            <div className="collab-avatar-big" style={{ background: note.owner.color }}>{note.owner.username[0].toUpperCase()}</div>
          ...
          <div className="modal-section">
            ...
            {note.collaborators.map(c => (
              <div key={c._id} className="collab-item">
                <div className="collab-avatar-big" style={{ background: c.user?.color }}>{c.user?.username?.[0]?.toUpperCase()}</div>
                <div className="collab-info">
                  <span className="collab-name">{c.user?.username}</span>
                  <span className="collab-email">{c.user?.email}</span>
                </div>
                <span className={`perm-badge ${c.permission}`}>{c.permission === 'write' ? 'Can edit' : 'Can read'}</span>
                <button className="remove-collab-btn" onClick={() => handleRemove(c.user._id)}>×</button>
              </div>
            ))}
          </div>
        )}
